...
.NOTES
  This creates a server in your Azure subscription with a Caddy proxy.
...
$vmOsDisk = "osdiskvmweb$numericSuffix"
...
$pipDnsName = "ws$Environment$numericSuffix-$OrgId".ToLowerInvariant()

$pipv4Name = "pipv4-$vmName-$Environment-$Location-01".ToLowerInvariant()
$pipv4DnsName = "ws$Environment$($numericSuffix)v4-$OrgId".ToLowerInvariant()
...
$privateIpSuffix = (0x1400 + $ServerNumber).ToString("x")
$privateIPv4Suffix = 40 + $ServerNumber
